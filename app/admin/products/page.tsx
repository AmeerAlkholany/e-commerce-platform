...
function AddModal({ onClose, onSubmit, form, setForm, categories, imagePreview, isUploading, uploadProgress, addingProduct, handleImageUpload, fileInputRef }: any) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="bg-luxe-surface border border-luxe-outline-variant/30 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={onSubmit} className="p-6 space-y-4">
          <div className="space-y-1.5">
            ...
            <input required value={form.name} onChange={e => setForm((p:any) => ({ ...p, name: e.target.value }))} type="text" className="w-full bg-luxe-surface-container border border-luxe-outline-variant/30 rounded-lg px-4 py-2.5 text-white outline-none focus:border-luxe-primary" />
          </div>
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold tracking-widest text-luxe-on-surface-variant uppercase">Description</label>
            <textarea value={form.description} onChange={e => setForm((p:any) => ({ ...p, description: e.target.value }))} rows={3} className="w-full bg-luxe-surface-container border border-luxe-outline-variant/30 rounded-lg px-4 py-2.5 text-white outline-none focus:border-luxe-primary resize-none" />
          </div>
          <div className="grid grid-cols-2 gap-4">
             <div className="space-y-1.5">
               <label className="text-[10px] font-bold tracking-widest text-luxe-on-surface-variant uppercase">Price *</label>
               <input required type="number" step="0.01" value={form.price} onChange={e => setForm((p:any) => ({ ...p, price: e.target.value }))} className="w-full bg-luxe-surface-container border border-luxe-outline-variant/30 rounded-lg px-4 py-2.5 text-white outline-none focus:border-luxe-primary" />
             </div>
             <div className="space-y-1.5">
               <label className="text-[10px] font-bold tracking-widest text-luxe-on-surface-variant uppercase">Stock</label>
               <input type="number" value={form.stock} onChange={e => setForm((p:any) => ({ ...p, stock: e.target.value }))} className="w-full bg-luxe-surface-container border border-luxe-outline-variant/30 rounded-lg px-4 py-2.5 text-white outline-none focus:border-luxe-primary" />
             </div>
          </div>
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold tracking-widest text-luxe-on-surface-variant uppercase">Category</label>
            <select value={form.category_id} onChange={e => setForm((p:any) => ({ ...p, category_id: e.target.value }))} className="w-full bg-luxe-surface-container border border-luxe-outline-variant/30 rounded-lg px-4 py-2.5 text-white outline-none focus:border-luxe-primary">
               <option value="">Select Category...</option>
               {categories.map((c:any) => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
          </div>
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold tracking-widest text-luxe-on-surface-variant uppercase">Image</label>
            <div onClick={() => fileInputRef.current?.click()} className="relative h-40 border-2 border-dashed border-luxe-outline-variant/30 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:bg-luxe-primary/5 transition-colors overflow-hidden">
               <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleImageUpload} />
               {imagePreview ? <img src={imagePreview} className="absolute inset-0 w-full h-full object-cover" alt="Preview" /> : <div className="flex flex-col items-center gap-2"><Plus className="size-6 text-luxe-primary" /><span className="text-xs text-luxe-on-surface-variant">Upload Artwork</span></div>}
               {isUploading && (
                  <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center p-6 space-y-3">
                    <Loader2 className="size-6 text-luxe-primary animate-spin" />
                    <div className="w-full bg-white/20 h-1 rounded-full overflow-hidden"><motion.div className="bg-luxe-primary h-full" initial={{ width: 0 }} animate={{ width: `${uploadProgress}%` }} /></div>
                  </div>
               )}
            </div>
          </div>
          <div className="flex gap-4 pt-4 border-t border-luxe-outline-variant/20">
            ...
            <Button type="submit" disabled={addingProduct} className="flex-1 bg-luxe-primary text-luxe-on-primary">{addingProduct ? <Loader2 className="animate-spin size-4" /> : "Link Product"}</Button>
          </div>
        </form>
      </motion.div>
    </motion.div>
