  ...
  return (
    ...
    >
      ...
      <motion.div variants={STAGGER.item} className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        <Card className="glass-panel border-none p-10 flex flex-col items-center justify-center text-center bg-gradient-to-br from-white/5 to-transparent relative overflow-hidden group">
          ...
          <div className="mt-6 flex items-center gap-2">
             <span className={cn(
                "px-3 py-0.5 text-[9px] font-black rounded uppercase tracking-widest border",
                isAdmin 
                  ? "bg-purple-500/10 text-purple-400 border-purple-500/20" 
                  : "bg-luxe-primary/10 text-luxe-primary border-luxe-primary/20"
             )}>
                {isAdmin ? "System Administrator" : "Verified Client"}
             </span>
             <span className="px-3 py-0.5 bg-sky-500/10 text-sky-400 text-[9px] font-black rounded uppercase tracking-widest border border-sky-500/20 flex items-center gap-1.5"><Sparkles className="size-2.5" /> {isAdmin ? "Full Access" : "Platinum"}</span>
          </div>
        ...
        <div className="lg:col-span-2 space-y-6">
           <h3 className="text-xs font-black uppercase tracking-[0.3em] text-luxe-on-surface-variant flex items-center gap-3">
              <TrendingUp className="size-3 text-luxe-primary" /> {isAdmin ? "Operational Overview" : "Portfolio Performance"}
           </h3>
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <MetricScorecard
                 label={isAdmin ? "System Actions" : "Acquisitions"}
                 value={isAdmin ? "642" : (stats?.totalOrders || 0)} // Dynamic dummy or real
                 icon={isAdmin ? <Briefcase className="size-5" /> : <Package className="size-5" />}
              />
              <MetricScorecard
                 label={isAdmin ? "Uptime Status" : "Delivered"}
                 value={isAdmin ? "100%" : (stats?.completedOrders || 0)}
                 icon={isAdmin ? <ShieldCheck className="size-5 text-emerald-400" /> : <CheckCircle2 className="size-5 text-emerald-400" />}
              />
              <MetricScorecard
                 label={isAdmin ? "Pending Task Load" : "Active Tasks"}
                 value={isAdmin ? "12" : (stats?.activeOrders || 0)}
      ...
      <motion.div variants={STAGGER.item} className="pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
         ...
         <p className="text-[10px] text-luxe-on-surface-variant/40 italic text-center md:text-right">
